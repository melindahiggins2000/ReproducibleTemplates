...
## [OPTIONAL] Intall LaTeX - to create PDF documents
...
3. Create a new Github repository "MyFirstDocument" with a Readme page - Click "Clone or Download" and In the "Clone with HTTPS" click to copy the URL to the clipboard
4. Open RStudio - create a New Project with Version Control - paste in the URL for the Github repository you just created. Choose a good location on your local system to save your project (e.g. "C:\MyGithub\MyFirstDocument").
5. In RStudio, create a new `rmarkdown` file - /File/New File/R Markdown/ - choose Document - HTML format put in a title and your name - click OK.
6. For now, you don't need to change anything. Click down arrow next to "Knit" and choose "Knit to HTML". The first time you try this, RStudio may ask you to install `rmarkdown` or `knitr` or other necessary packages.
7. If that works, also try "Knit to Word" to try creating a DOC formatted document. 
8. [OPTIONAL] If you have LaTeX installed, you can also try "Knit to PDF".
...
* We can do this using the RStudio interface, but this can be slow and often does not work well when there are many changes and many files that need to be synced. 
* Instead, use Git Bash to: 
    a. change directory to your new Github repository with your new RStudio project
    b. add the files you want to back up in the Github repository ("staging")
    c. commit the files you want to back up ("snapshot of your files") - add a descriptive comment about your changes and updates
    d. push the files to Github [PUSH indicates copying files from your local system up to the Github repository "cloud"]
    e. check git status
...
|:-----|:---------------------|:-------------------------|
| 1 | `$ cd /c/MyGithub/MyFirstDocument` | change directory to your project folder |
